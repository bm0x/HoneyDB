...
namespace L_TEC_BIC_JFERSOLAR
{
    public partial class Ing_Empresa_Practica : Form
    {
        ...
        private void Ing_Empresa_Practica_Load(object sender, EventArgs e)
        {
        ...
        private void Img_Grabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Debe ingresar el código de la Empresa para Practica");
                    Txt_codigo.Focus();
                    return;
                }
                ...
                if (Txt_Desc.Text == "")
                {
                    MessageBox.Show("Debe ingresar la descripción de la Empresa para Practica");
                    Txt_Desc.Focus();
                    return;
                }
                ...
                //Aquí comienza el código que permitirá guardar los datos en la tabla

                String query = "select count(*) from Empresa_Practica where Cod_Empre_Prac='" + Txt_codigo.Text + "'";
                ...
                if (existe == 0)
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("INSERT INTO Empresa_Practica (Cod_Empre_Prac, Desc_Empre_Prac) " +
                        "VALUES('" + Txt_codigo.Text + "','" + Txt_Desc.Text + "')", con);
                    cmd1.ExecuteNonQuery();

                    MessageBox.Show("Empresa Practica Ingresada Exitosamente");
                ...
                else
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("Update Empresa_Practica set Desc_Empre_Prac ='" + Txt_Desc.Text + "' " +
                        "where Cod_Empre_Prac ='" + Txt_codigo.Text + "'", con);
                    cmd1.ExecuteNonQuery();
        ...
        private void Img_Eliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Para eliminar una Empresa Practica debe ingresar a lo menos el código");
                    Txt_codigo.Focus();
                    return;
                }
                else
                {
                    ...
                    if (existe != 0)
                    {
                        ...
                        String query1 = "select count(*) from Egresado where Cod_Empre_Prac='" + Txt_codigo.Text + "'";
                        ...
                        if (existe == 0)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("Delete From Empresa_Practica  where Cod_Empre_Prac ='" + Txt_codigo.Text + "'", con);
                            cmd1.ExecuteNonQuery();
                            MessageBox.Show("Empresa Practica Eliminada Exitosamente");
                        }
                        else
                        {
                            MessageBox.Show("No puede Eliminar la Empresa Practica esta asociado a un Egresado");
                        }

                    }
                    else
                    {
                        MessageBox.Show("Empresa Practica no existe para Eliminar");
                    }
                    con.Close();
