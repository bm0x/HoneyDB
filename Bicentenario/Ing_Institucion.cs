...
namespace L_TEC_BIC_JFERSOLAR
{
    public partial class Ing_Institucion : Form
    {
        ...
        private void Ing_Institucion_Load(object sender, EventArgs e)
        {
        ...
        private void Img_Grabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Debe ingresar el código de la Institucion");
                    Txt_codigo.Focus();
                    return;
                }
                ...
                if (Txt_Desc.Text == "")
                {
                    MessageBox.Show("Debe ingresar la descripción de la Institucion");
                    Txt_Desc.Focus();
                    return;
                }
                ...
                String query = "select count(*) from Institucion where Cod_Inst='" + Txt_codigo.Text + "'";
                ...
                if (existe == 0)
                {
                    //En esta parte se ingresa la nueva institucion usando la instruccion Insert
                    SqlCommand cmd1 = new SqlCommand("INSERT INTO Institucion (Cod_Inst,Desc_Inst) " +
                        "VALUES('" + Txt_codigo.Text + "','" + Txt_Desc.Text + "')", con);
                    cmd1.ExecuteNonQuery();

                    MessageBox.Show("Institucion Ingresada Exitosamente");
                ...
                else
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("Update Institucion set Desc_Inst ='" + Txt_Desc.Text + "' " +
                        "where Cod_Inst ='" + Txt_codigo.Text + "'", con);
                    cmd1.ExecuteNonQuery();
        ...
        private void Img_Eliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Para eliminar una Institucion debe ingresar a lo menos el código");
                    Txt_codigo.Focus();
                    return;
                }
                else
                {
                    ...
                    if (existe != 0)
                    {
                        ...
                        String query1 = "select count(*) from Institucion_Carrera where Cod_Inst='" + Txt_codigo.Text + "'";
                        ...
                        if (existe == 0)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("Delete From Institucion  where Cod_Inst ='" + Txt_codigo.Text + "'", con);
                            cmd1.ExecuteNonQuery();
                            MessageBox.Show("Institucion Eliminada Exitosamente");
                        }
                        else
                        {
                            MessageBox.Show("No puede Eliminar la institucion esta asociada a una Carrera");
                        }

                    }
                    else
                    {
                        MessageBox.Show("Institucion no existe para Eliminar");
                    }
                    con.Close();
