...
namespace L_TEC_BIC_JFERSOLAR
{
    public partial class Ing_Est_Egre : Form
    {
        ...
        private void Ing_Est_Egre_Load(object sender, EventArgs e)
        {
        ...
        private void Img_Grabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Debe ingresar el código del Estado");
                    Txt_codigo.Focus();
                    return;
                }
                ...
                if (Txt_Desc.Text == "")
                {
                    MessageBox.Show("Debe ingresar la descripción del Estado");
                    Txt_Desc.Focus();
                    return;
                }
                ...
                String query = "select count(*) from Estado_Egresado where Cod_Est_Egre='" + Txt_codigo.Text + "'";
                ...
                if (existe == 0)
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("INSERT INTO Estado_Egresado (Cod_Est_Egre, Desc_Est_Egre) " +
                ...
                else
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("Update Estado_Egresado set Desc_Est_Egre ='" + Txt_Desc.Text + "' " +
                        "where Cod_Est_Egre ='" + Txt_codigo.Text + "'", con);
                    cmd1.ExecuteNonQuery();
        ...
        private void Img_Eliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo.Text == "")
                {
                    MessageBox.Show("Para eliminar un Estado debe ingresar a lo menos el código");
                    Txt_codigo.Focus();
                    return;
                }
                else
                {
                    ...
                    if (existe != 0)
                    {
                        ...
                        String query1 = "select count(*) from Egresado where Cod_Est_Egre='" + Txt_codigo.Text + "'";
                        ...
                        if (existe == 0)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("Delete From Estado_Egresado  where Cod_Est_Egre ='" + Txt_codigo.Text + "'", con);
                            cmd1.ExecuteNonQuery();
                            MessageBox.Show("Estado Eliminado Exitosamente");
                        }
                        else
                        {
                            MessageBox.Show("No puede Eliminar el Estado esta asociado a un Egresado");
                        }

                    }
                    else
                    {
                        MessageBox.Show("Estado no existe para Eliminar");
                    }
                    con.Close();
